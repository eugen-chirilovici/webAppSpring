...
public class UserController {
    ...
    public String submit(@ModelAttribute("credentials") Credentials credentials) {

        Long userId = authenticationService.confirmAuthentication(credentials);
        loggedUser = userService.getUserById(userId);

        if (loggedUser != null) {
            if (loggedUser.getRole().equals(RoleType.ROLE_ADMIN)) {
                return "redirect:/allusers";
            } else if (loggedUser.getRole().equals(RoleType.ROLE_USER)) {
